...
def createLabelUNet(img,divNum,netMask,CropSize,fillsize,ctxMask=mx.cpu(0),ctxInter=mx.cpu(0),netInter=None):
    ...
    for sub in Subs:
        OrgSizeX=len(sub[0])
        OrgSizeY=len(sub)                             
        if (len(sub)<CropSize)or(len(sub[0])<CropSize):
            sub=nd.expand_dims(mx.nd.array(sub), 0)
            sub=mx.img.imresize(sub,CropSize,CropSize)
            sub = sub[0, :, :]    
        sub=nd.expand_dims(mx.nd.array(sub), 0)
        DetectionSubs.append(predictionResize(sub,netMask,OrgSizeX,OrgSizeY,ctxMask))
        if netInter!=None:
            IntersectionSubs.append(predictionResize(sub,netInter,OrgSizeX,OrgSizeY,ctxInter))
